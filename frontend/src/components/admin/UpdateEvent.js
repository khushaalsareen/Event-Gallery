...
function UpdateEvent() {
  ...
  async function handleCreateEvent() {
    ...
    if (createEvent.eventName === '' && createEvent.startDate === '' && createEvent.endDate === '' && createEvent.eventdetails === '' && createEvent.eventCoordinators === '' && createEvent.eventType==='') {
      alert("Please fill any one Detail to update")
    } else {
      axios({
        method: "put",
        url: `http://localhost:8000/api/event/updateEvent/${name}`,
        data: createEvent,
              ...
              <div className="mx-auto w-full max-w-[550px] bg-white text-black p-9 rounded-xl">
                ...
                <div className="mb-5">
                  ...
                  <input
                    ...
                    className="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
                    value={createEvent.eventFee} onChange={(e) => { setCreateEvent({ ...createEvent, eventFee: e.target.value }) }}
                  />
                </div>
                {/*  */}
                <div>
